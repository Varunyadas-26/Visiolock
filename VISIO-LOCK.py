...
class FaceDetectionApp(QWidget):
    ...
    def updateFrame(self):
        ...
        global alertstring, openstring, alert_sent  # Include alert_sent in global variables
                ...
                if username == "admin":
                    ...
                    if len(faces) > 0:
                        for face in faces:
                            ...
                            if identity == "Unknown" and not alert_sent:
                                current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                                cv2.imwrite(f"unknown_{current_time}.jpg", frame)
                                self.alertUnknownPerson(current_time)
                                alert_sent = True  # Set alert_sent to True to prevent multiple alerts
...
def on_callback_query(msg):
    global sentcommand, alert_sent  # Include alert_sent in global variables
    ...
    if query_data == "accept":
        ...
        alert_sent = False  # Reset alert_sent flag after action
